...
def parse_line(line: str):
    ...
    try:
        ...
        az = float(parts[4])
        mag = float(parts[5])
        return time_ms, temp_c, ax, ay, az, mag
...
def main():
    ...
    parser.add_argument("--window", type=float, default=10.0, help="Plot window in seconds")
    ...
    fig.suptitle("ESP32-C3 Smart Reflector Sensor Data")
    ...
    def update(_frame):
        ...
        while ser.in_waiting and lines_read < 25:
            ...
            time_ms, temp_c, ax, ay, az, mag = parsed
        ...
        if len(t_data) < 2:
            return line_temp, line_x, line_y, line_z, line_mag
        ...
        if mag_data:
            ax_mag.set_ylim(min(mag_data) - 2, max(mag_data) + 2)
